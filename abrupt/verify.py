...
"""Verify exact lower and upper bounds for 3x3 Abrupt Dark Hex."""
...
CERTIFICATES = (
    ("black-lower.mix", BLACK, Fraction(207579464761, 299876201866)),
    ("white-upper.mix", WHITE, Fraction(207579464761, 299876201866)),
)
...
def append_event(history, cell, success):
    if history >> 40:
        raise ValueError("private history exceeds nine probes")
...
class BestResponder:
    ...
    def advance_opponent(self, world):
        ...
        if (black | white) & cell_bit:
            # A collision ends the opponent's turn.
    ...
    def expand(self, action, worlds):
        ...
        for world in worlds:
            ...
            if opponent_stones & action_bit:
                # A collision ends the responder's turn.
                terminal, next_world = self.advance_opponent(world)
                if not terminal:
                    collision.append(next_world)
                continue
            own |= action_bit
            if self.me == BLACK:
                black = own
    ...
    def value(self, history, worlds):
        ...
        # The responder's knowledge and the consistent hidden worlds determine
        # every future payoff.
...
def main():
    ...
    print("abrupt_3x3_certificate ok")
